...
install_pre() {
    ...
    if [[ ${#xcpretty_exist} == 0 || $xcpretty_exist != "$pre_name ("* ]]; then
        echo "\033[33;1m未安装 $pre_name, 将自动安装\033[0m"
        sudo gem install $pre_name
        echo "\033[33;1m安装 $pre_name 成功 \033[0m"
        echo ""
    else
        echo "\033[33;1m已安装 $pre_name \033[0m"
    fi
}
...
workspage_bundle_id=`echo "$info" | grep PRODUCT_BUNDLE_IDENTIFIER | awk -F ' = ' '{print $2}'`
workspage_team_id=`echo "$info" | grep DEVELOPMENT_TEAM | awk -F ' = ' '{print $2}'`
scheme_name=`echo "$info" | grep TARGET_NAME | awk -F ' = ' '{print $2}'`
echo "\033[33;1mBundle ID: $workspage_bundle_id"
echo "Team ID: $workspage_team_id"
echo "Scheme Name: $scheme_name \033[0m"

# ===================================
# 设置指定Xcode
get_xcode_path

echo "\033[32;1m设置指定Xcode...\033[0m"

if [[ `expr ${#xcode_paths[@]}` == 1 ]]; then
    number=0
else
    echo "\033[33;1m请输入数字选择指定的 Xcode\033[0m"
    for i in "${!xcode_paths[@]}";   
    do
        echo "${i}: ${xcode_paths[$i]}"
    done
    read -r -t 5 -n1 number
fi

length=`expr ${#xcode_paths[*]} - 1`
if [[ $number > $length || $number < 0 ]]; then
    echo "\033[31;1m错误的编号, 将使用默认路径: `xcode-select -p`\033[0m"
    number=0
fi

selected_xcode_path="${xcode_paths[$number]}"
echo "\033[33;1m输入密码以设置 Xcode"
sudo xcode-select -s "$selected_xcode_path/Contents/Developer"
echo "已设置为指定 Xcode: `xcode-select -p`\033[0m"

# ===================================
# 设置基本信息
echo "\033[32;1m设置基本信息...\033[0m"
# method，打包的方式。方式分别为 development, ad-hoc, app-store, enterprise 。必填
echo "请输入打包方式编号 \033[33;1m [1:app-store 2:ad-hoc 3:development 4:enterprise]\033[0m"
...
if $is_upload_dSYM_Bugly ; then
    # 准备上传dSYM文件到 bugly
    echo "\033[33m准备上传dSYM文件到 Bugly... \033[0m"
    ...
    echo "\033[33m上传dSYM文件到 Bugly 成功 \033[0m"
fi
...
# 上传 App Store
if [ $number == 1 ];then
echo "\033[33m准备上传ipa到App Store... \033[0m"
...
echo "\033[33m上传ipa到App Store成功 \033[0m"
fi

# 恢复默认 xcode-select
if [[ `expr ${#xcode_paths[@]}` > 1 ]]; then
    echo "\033[32;1m恢复默认 xcode-select \033[0m"
    sudo xcode-select --reset
fi

# 打开输出目录
open "$export_path"
echo "\033[32;1m全部完成 🎉🎉🎉\033[0m"
exit 0